...
function OverviewCardMesh({ data, circleIcon }) {
  ...
  return (
    <div
      className={`relative p-[25px] 3xl:p-5 after:absolute after:top-1/2 after:-translate-y-1/2 ltr:after:-right-[60px] ltr:3xl:after:right-0 rtl:after:-left-[60px] rtl:3xl:after:left-0 after:w-[1px] after:h-[70px] xs:after:hidden xs-w-full after:bg-regular dark:after:bg-white10 last:after:hidden ${
    ...
    >
      <div>
        <div className="flex items-center gap-[25px] 3xl:gap-[15px]">
          <div
            className={`flex items-center justify-center w-[70px] 3xl:w-[50px] h-[70px] 3xl:h-[50px] rounded-full text-white bg-${type}`}
          >
            <ReactSVG
              src={require(`../../static/img/icon/${icon}`)}
              className="3xl:[&>div>svg]:w-5 3xl:[&>div>svg]:h-5"
            />
          </div>
